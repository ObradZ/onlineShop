...
//	    Taking parameters from register.jsp
	    String username = request.getParameter("username");
	    String password = request.getParameter("password");

//	    Security check for taken parameters
...
//	    Everything is ok, creating User!
		try {
		    Session session = HibernateUtil.createSessionFactory().openSession();
		    Query query = session.createSQLQuery("insert into user values(null,'" + username + "',password('" + password + "'),0)");
		    query.executeUpdate();
		    if (session.isOpen()) {
			session.close();
		    }
